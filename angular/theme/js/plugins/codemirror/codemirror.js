...
(function (mod) {
    ...
    function updateLineForChanges(cm, lineView, lineN, dims)
    {
        for (var j = 0; j < lineView.changes.length; j++) {
            ...
            if (type == "text") updateLineText(cm, lineView);
                ...
                if (type == "gutter") updateLineGutter(cm, lineView, lineN, dims);
                else
                    if (type == "class") updateLineClasses(lineView);
                    else
                        if (type == "widget") updateLineWidgets(lineView, dims);
        }
    ...
    function coordsCharInner(cm, lineObj, lineNo, x, y)
    {
        ...
        function getX(ch)
        {
            ...
            if (innerOff > sp.bottom) return sp.left - adjust;
                ...
                if (innerOff < sp.top) return sp.left + adjust;
    ...
    function regChange(cm, from, to, lendiff)
    {
        ...
        if (from >= display.viewTo) { // Change after
            ...
            if (to <= display.viewFrom) { // Change before
                ...
                if (from <= display.viewFrom && to >= display.viewTo) { // Full overlap
                    resetView(cm);
                } else
                    if (from <= display.viewFrom) { // Top overlap
                        ...
                        if (to >= display.viewTo) { // Bottom overlap
    ...
    function readInput(cm)
    {
        ...
        if (cm.state.pasteIncoming && doc.sel.ranges.length > 1) {
            if (lastCopied && lastCopied.join("\n") == inserted)
                ...
                if (textLines.length == doc.sel.ranges.length)
        ...
        for (var i = doc.sel.ranges.length - 1; i >= 0; i--) {
            ...
            // Handle overwrite
                ...
                if (cm.state.overwrite && range.empty() && !cm.state.pasteIncoming)
    ...
    if (ie) wheelPixelsPerUnit = -.53;
        ...
        if (gecko) wheelPixelsPerUnit = 15;
        else
            if (chrome) wheelPixelsPerUnit = -.7;
            else
            ...
            if (display.input.selectionStart != null) {
                ...
                var i = 0, poll = function () {
                    if (display.selForContextMenu == cm.doc.sel && display.input.selectionStart == 0)
                        ...
                        if (i++ < 10) display.detectingSelectAll = setTimeout(poll, 500);
                        else resetInput(cm);
                };
    ...
    CodeMirror.prototype = {
        ...
        getTokenTypeAt: function (pos) {
            ...
            if (ch == 0) type = styles[2];
                ...
                if ((mid ? styles[mid * 2 - 1] : 0) >= ch) after = mid;
                    ...
                    if (styles[mid * 2 + 1] < ch) before = mid + 1;
                    else {
                        type = styles[mid * 2 + 2];
                        break;
                    }
        ...
        cursorCoords: function (start, mode) {
            ...
            if (start == null) pos = range.head;
                ...
                if (typeof start == "object") pos = clipPos(this.doc, start);
    ...
    Doc.prototype = createObj(BranchChunk.prototype, {
        ...
        getCursor: function (start) {
            ...
            if (start == null || start == "head") pos = range.head;
                ...
                if (start == "anchor") pos = range.anchor;
                else
                    if (start == "end" || start == "to" || start === false) pos = range.to();
                    else pos = range.from();
            return pos;
        },
        ...
        addLineClass: docMethodOp(function (handle, where, cls) {
            return changeLine(this, handle, "class", function (line) {
                ...
                if (!line[prop]) line[prop] = cls;
                    ...
                    if (new RegExp("(?:^|\\s)" + cls + "(?:$|\\s)").test(line[prop])) return false;
                    else line[prop] += " " + cls;
                return true;
            });
        }),
    ...
    if (ios) // Mobile Safari apparently has a bug where select() is broken.
        ...
        if (ie) // Suppress mysterious IE10 errors
            selectInput = function (node) {
                try {
                    node.select();
                } catch (_e) {
    ...
    var bidiOrdering = (function () {
        ...
        function charType(code)
        {
            if (code <= 0xf7) return lowTypes.charAt(code);
                ...
                if (0x590 <= code && code <= 0x5f4) return "R";
                else
                    if (0x600 <= code && code <= 0x6ed) return arabicTypes.charAt(code - 0x600);
                    else
                        if (0x6ee <= code && code <= 0x8ac) return "r";
                        else
                            if (0x2000 <= code && code <= 0x200b) return "w";
                            else
        ...
        return function (str) {
            ...
            for (var i = 0, cur = outerType; i < len; ++i) {
                ...
                if (cur == "L" && type == "1") types[i] = "L";
                    ...
                    if (isStrong.test(type)) cur = type;
            }
