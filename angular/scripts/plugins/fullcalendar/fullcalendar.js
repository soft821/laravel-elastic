    ...
    function Calendar(element, instanceOptions)
    {
        ...
        { // assumes elementVisible
            if (typeof options.contentHeight === 'number') { // exists and not 'auto'
                ...
                if (typeof options.height === 'number') { // exists and not 'auto'
                    ...
                    $.each(this.split(','), function (j, buttonName) {
                        ...
                        if (buttonName == 'title') {
                            ...
                            if (calendar[buttonName]) { // a calendar method
                                ...
                                if (fcViews[buttonName]) { // a view name
                                    buttonClick = function () {
	...
	-----------------------------------------------------------------------------*/
        ...
        function clientEvents(filter)
        {
            if ($.isFunction(filter)) {
                ...
                if (filter != null) { // not null, not undefined. an event ID
                    filter += '';
        ...
        if (moment.isMoment(input)) {
            ...
            if (isNativeDate(input) || input === undefined) {
                ...
                if (isAmbigTime) {
                    ...
                    if (parseZone) { // let's record the inputted zone somehow
                        if (isAmbigZone) {
                            output._ambigZone = true;
                        }
                        else
...
// addition formatting tokens we want recognized
    ...
    function formatDateWithChunk(date, chunk)
    {
        ...
        if (typeof chunk === 'string') { // a literal string
            ...
            if ((token = chunk.token)) { // a token, like "YYYY"
                ...
                if (chunk.maybe) { // a grouping of other chunks that must be non-zero
    ...
    function chunkFormatString(formatStr)
    {
        ...
        while ((match = chunker.exec(formatStr))) {
            if (match[1]) { // a literal string inside [ ... ]
                ...
                if (match[2]) { // non-zero formatting inside ( ... )
                    chunks.push({maybe: chunkFormatString(match[2])});
                }
                else
                    if (match[3]) { // a formatting token
                        chunks.push({token: match[3]});
                    }
                    else
                        if (match[5]) { // an unenclosed literal string
                            chunks.push(match[5]);
                        }
        }
	...
	------------------------------------------------------------------------------------------------------------------*/
        ...
        constrainScrollVel: function () {
            ...
            if (this.scrollTopVel < 0) { // scrolling up?
                ...
                if (this.scrollTopVel > 0) { // scrolling down?
            ...
            if (this.scrollLeftVel < 0) { // scrolling left?
                ...
                if (this.scrollLeftVel > 0) { // scrolling right?
            ...
            return $('<a class="fc-more"/>')
                ...
                .on('click', function (ev) {
                    ...
                    if (clickOption === 'popover') {
                        ...
                        if (typeof clickOption === 'string') { // a view name
