    ...
    CodeMirror.defineMode("markdown", function (cmCfg, modeCfg) {
        ...
        function blockNormal(stream, state)
        {
            ...
            if (state.list !== false && state.indentationDiff >= 0) { // Continued list
                ...
                if (state.list !== false && state.indentation > 0) {
                    state.list = null;
                    state.listDepth = Math.floor(state.indentation / 4);
                } else
                    if (state.list !== false) { // No longer a list
                        state.list = false;
                        state.listDepth = 0;
                    }
                        ...
                        if (state.prevLineHasContent && (match = stream.match(setextHeaderRE))) {
                            ...
                            if (stream.eat('>')) {
                                ...
                                if (stream.peek() === '[') {
                                    ...
                                    if (stream.match(hrRE, true)) {
                                        return hr;
                                    } else
                                        if ((!state.prevLineHasContent || prevLineIsList) && (stream.match(ulRE, false) || stream.match(olRE, false))) {
                                            var listType = null;
                                            ...
                                            if (modeCfg.fencedCodeBlocks && stream.match(/^```([\w+#]*)/, true)) {
                                                // try switching mode
            ...
            if (ch === '*' || (ch === '_' && !ignoreUnderscore)) {
                if (sol && stream.peek() === ' ') {
                    ...
                    if (state.strong === ch && stream.eat(ch)) { // Remove STRONG
                        ...
                        if (!state.strong && stream.eat(ch)) { // Add STRONG
                            state.strong = ch;
                            if (modeCfg.highlightFormatting) state.formatting = "strong";
                            return getType(state);
                        } else
